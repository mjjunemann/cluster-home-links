...
SCRIPT = """
echo "{password}" | sudo -S rm {dest}/{user}
"""
...
def create_unlink_script(user, destination_folder, password):
...
def main(args, passwd):
    ...
    if not groups and args.group in groups:
        print("no hosts specified or group not valid")
        return 0
    ...
    scripts = []
    script_name = create_unlink_script(
        args.user, args.destination, passwd
    )
